// cefr_data.js — Oxford + CEFR-J seviye haritası
// Kaynak: CEFR-J Vocabulary Profile & Octanove Vocabulary Profile (C1/C2)
// Format: { "word": "A1" | "A2" | "B1" | "B2" | "C1" | "C2" }
var CEFR_MAP = {
    "a": "A1",
    "abandon": "B1",
    "abandoned": "B2",
...
function lookupCEFR(word) {
    ...
    if (!w)
        return null;
    // Düzensiz çekimleri (irregular forms) kök haline getir
    if (IRREGULAR_FORMS[w]) {
        w = IRREGULAR_FORMS[w];
    }
    if (CEFR_MAP[w])
        return CEFR_MAP[w];
    // Çekim eklerini soyarak kök adayları oluştur
    const candidates = [];
    // 1. Çoğullar (-s, -es, -ies)
    ...
    // 2. Fiil çekimleri (-ing, -ed)
    if (w.endsWith('ing') && w.length > 5) {
        const base = w.slice(0, -3);
        candidates.push(base);
        candidates.push(base + 'e');
        // Çift ünsüzü teke düşür (running -> run)
    ...
    if (w.endsWith('ed') && w.length > 4) {
        ...
        candidates.push(base);
        candidates.push(base + 'e');
        // Çift ünsüzü teke düşür (stopped -> stop)
        if (base.length > 2 && base[base.length - 1] === base[base.length - 2]) {
            candidates.push(base.slice(0, -1));
        }
    }
    ...
    // 4. Karşılaştırma sıfatları (-er, -est)
    ...
    // Adayları kontrol et, ilk eşleşen seviyeyi döndür
    for (const c of candidates) {
        let cleanCandidate = c;
        if (IRREGULAR_FORMS[cleanCandidate]) {
            cleanCandidate = IRREGULAR_FORMS[cleanCandidate];
        }
        if (CEFR_MAP[cleanCandidate]) {
            return CEFR_MAP[cleanCandidate];
        }
    }
    return null;
}
